...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (username, password) => {
    try {
      ...
      const res = await axios.post('http://localhost:8000/api/v1/users/auth/login', {
        username,
        password
      ...
      if (res.status === 200) {
        ...
        const isUserAdmin = res.data.user.isAdmin;
        // console.log("hi");
  ...
  // Change password function (for logged-in user)
  const changePassword = async (oldPassword, newPassword) => {
    try {
      const accessToken = localStorage.getItem('accessToken');
      
      const response = await axios.post('http://localhost:8000/api/v1/users/auth/change-password', 
        { 
          oldPassword, 
          newPassword 
        }, 
  ...
  const adminResetPassword = async (userId, newPassword) => {
    try {
      ...
      const response = await axios.post(`http://localhost:8000/api/v1/users/admin/users/${userId}/reset-password`, 
        { 
          newPassword 
        }, 
        {
  ...
  const logout = async () => {
    try {
      ...
      await axios.post('http://localhost:8000/api/v1/users/auth/logout', {}, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
        withCredentials: true
