...
function History() {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:8000/api/v1/users/issues/get-issue-for-user', {
      headers: {
        Authorization: `Bearer ${accessToken}`,
      },
      withCredentials: true
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:8000/api/v1/users/issues/get-issue', {
      headers: {
        Authorization: `Bearer ${accessToken}`,
      }, withCredentials: true
    })
  ...
  const handleComplete = async (taskId) => {
    ...
    try {
      await axios.post('http://localhost:8000/api/v1/users/issues/complete-report', {
        issueId: taskId
      }, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
  ...
  const handleAcknowledge = async (taskId) => {
    ...
    try {
      await axios.post('http://localhost:8000/api/v1/users/issues/acknowledge-time', {
        responseId: taskId
      }, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
          ...
          <div className="flex flex-col lg:flex-row lg:space-x-6 justify-center items-start">
            ...
            <div className="lg:w-1/2 w-full bg-white border border-gray-200 rounded-lg shadow-xl p-4 mb-6 lg:mb-0 max-sm:p-2 max-sm:mb-4">
              ...
              <div className="max-h-[26rem] overflow-y-auto p-4 max-sm:p-2">
                {(toResolvetasks && toResolvetasks.length > 0) ? (
